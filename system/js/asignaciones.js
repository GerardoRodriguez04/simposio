$(document).ready(function() {
    ...
    $(".form-solicitudes").on("submit", function(e) {
    	...
    	$.ajax({
    		url: base_url+'asignaciones/form_solicitud',
    		type: 'POST',
    		dataType: 'JSON',
			data: data,
			cache: false,
			...
			success: function(res){
				...
				$('.alert-success').show();
				setTimeout(function(){ window.location.replace(base_url+'asignaciones'); }, 2000);
			}
    	});
    });
...
function validar_form(){
    ...
    $.ajax({
        url: base_url + "asignaciones/validar_form",
        type: "POST",
        dataType: 'JSON',
        data: data,
		cache: false,
...
function crear_paginacion(pagina){
	$.ajax({
		url: base_url+'asignaciones/get_solicitudes/'+pagina,
		type: 'GET',
		dataType: 'JSON',
...
function listar_tabla(result, row){
	...
	$.each(result, function(index, val) {
		...
		if(val['documento'] !=  null){
			documento = '<a class="dropdown-item" href="'+base_url+'asignaciones/descargar_documento_solicitud/'+val['solicitud_id']+'/'+val['documento']+'"><i class="fa fa-file-text-o" aria-hidden="true" style="color: #d50000;"></i> Descargar</a>';
		}

		// if($('.estatus_id').val() > 1){
			detalles = '<a class="dropdown-item" href="'+base_url+'asignaciones/detalles_view/'+val['solicitud_id']+'"><i class="fa fa-info-circle" aria-hidden="true" style="color: #00838f;"></i> Detalles</a>';
		// }
...
function servicio_solicitud(solicitud_id, dia, hora, minuto, estatus_id, mensaje) {
	...
	if (res == true) {
		$.ajax({
			async: true, 
			url: base_url+'asignaciones/servicio_solicitud',
			type: 'POST',
			dataType: 'JSON',
...
function autorizar_solicitud(solicitud_id, estatus_id){
	...
	if (res == true) {
		$.ajax({
			async: true, 
			url: base_url+'asignaciones/proceso_solicitud',
			type: 'POST',
			dataType: 'JSON',
...
function get_servicios(departamento_id , tipo){
	...
	$.ajax({
		url: base_url+'asignaciones/get_servicios',
		type: 'POST',
		dataType: 'JSON',
...
function get_usuarios(departamento_id ){
	...
	$.ajax({
		url: base_url+'asignaciones/get_usuarios',
		type: 'POST',
		dataType: 'JSON',
		data: {departamento_id: departamento_id},
		success: function(res){
...
function get_solicitud_detalle(solicitud_id){
	$.ajax({
		url: base_url+'asignaciones/get_solicitud_detalle',
		type: 'POST',
		dataType: 'JSON',
		...
		success: function(res){
			...
			if(res[0]['documento'] != null){
				documento = '&nbsp;&nbsp;<a href="'+base_url+'asignaciones/descargar_documento_solicitud/'+res[0]['solicitud_id']+'/'+res[0]['documento']+'" class="btn btn-outline-danger btn-xs btn-documento-solicitud"><i class="fa fa-file-text-o" aria-hidden="true"></i> Descargar</a>&nbsp;&nbsp;';
			}
...
function get_comentarios(solicitud_id){
	...
	$.ajax({
		url: base_url+'asignaciones/get_comentarios',
		type: 'POST',
		dataType: 'JSON',
		data: {solicitud_id: solicitud_id},
		success: function(res){
			$.each(res, function(index, val) {
				...
				if(val.documento != '' && val.documento != null){
					doc = '<a href="'+base_url+'asignaciones/descargar_documento/'+val.comentario_id+'/'+val.documento+'" class="badge badge-primary" style="background: #d05400 !important;"><i class="fa fa-file-code-o" aria-hidden="true"></i> Descargar</a>';
				}
...
function agregar_comentario(){
	...
	$.ajax({
		url: base_url+'asignaciones/agregar_comentario',
		type: 'POST',
		dataType: 'JSON',
		data: data,
		cache: false,
...
function get_documentos(solicitud_id){
	...
	$.ajax({
		url: base_url+'asignaciones/get_documentos',
		type: 'POST',
		dataType: 'JSON',
		data: {solicitud_id: solicitud_id},
		success: function(res){
				...
				if(val.documento != '' && val.documento != null){				
					...
					$('.div-documentos').append(
						'<div class="col-md-2 div-documento">'
			  				+'<a href="'+base_url+'asignaciones/descargar_documento/'+val.comentario_id+'/'+val.documento+'">'
